            ...
            steps {
                script {
                    withCredentials([sshUserPrivateKey(credentialsId: 'aws-ec2-key', keyFileVariable: 'SSH_KEY')]) {
                        ...
                        ssh -i "%SSH_KEY%" -o StrictHostKeyChecking=no ubuntu@65.2.40.135 "sudo docker pull ${DOCKER_HUB_USER}/${APP_NAME}:latest ; sudo docker stop weather-app ; sudo docker rm weather-app ; sudo docker run -d --name weather-app -p 5000:5000 -e WEATHER_API_KEY='6854f09a4c28f0eafbb8493321eaf6e3' ${DOCKER_HUB_USER}/${APP_NAME}:latest"
                        """
                    }
                }
            }
